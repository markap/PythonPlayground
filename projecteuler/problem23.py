...
As 12 is the smallest abundant number, 1 + 2 + 3 + 4 + 6 = 16, the smallest number
 ...
 though it is known that the greatest number that cannot be expressed as the sum of two 
...
for i in range(1, 28100):
    if sum([j for j in range(1, i/2 + 1) if i % j == 0]) > i:
        abandant_numbers.append(i)

print abandant_numbers    
